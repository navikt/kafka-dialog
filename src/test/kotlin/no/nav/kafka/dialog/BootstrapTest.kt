...
class BootstrapTest {
    ...
    fun setup() {
        ...
        kafkaConsumer = TestKafkaConsumer<String, String>()
    ...
    fun `SalesforceClient with NO_POST flag set should not do HTTP post`() {
        // arrange
        properties["POSTER_SETTINGS"] = "${properties["POSTER_SETTINGS"]}, NO_POST"
        val myValue = "values"
        setupAccessToken(OK)
        setupKafkaToProduce(myValue)
        setupSalesforceHTTPResponse(success = true)
    ...
    private fun setupSalesforceHTTPResponse(success: Boolean = true) =
        setupHTTPResponse(body = sfsObjectStatuses(success = success))
    ...
    private fun sfsObjectStatuses(success: Boolean) = """ [ { "id":"id","success":$success,"errors":[] } ] """
    ...
    inner class TestKafkaConsumer<K, V> : MockConsumer<K, V>(EARLIEST) {
